...
3. **Review tasks** — \`arcbridge_get_phase_plan\` to see if any tasks can be inferred as done
4. **Propose updates** — \`arcbridge_arc42\` (action: propose) to generate documentation update proposals
`;
...
5. **Complete phase** — if all checks pass, run \`arcbridge_manage_phases\` (action: complete) to validate gates and transition
`;
