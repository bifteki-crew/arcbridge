...
function generateAgentFile(role: AgentRole): string {
  ...
  // Map ArcBridge tools to Gemini MCP tool patterns
  if (role.required_tools.length > 0) {
    lines.push("tools:");
    for (const tool of role.required_tools) {
      lines.push(`  - mcp_arcbridge_${tool.replace("arcbridge_", "")}`);
    }
  }
...
export class GeminiAdapter implements PlatformAdapter {
  ...
  generateAgentConfigs(targetDir: string, roles: AgentRole[]): void {
    ...
    // duplication needed. If Codex isn't in platforms, generate skills here.
    const skillsDir = join(targetDir, ".agents", "skills");
    const syncSkillPath = join(skillsDir, "arcbridge-sync", "SKILL.md");
    if (!existsSync(syncSkillPath)) {
