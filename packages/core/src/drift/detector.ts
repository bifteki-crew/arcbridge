...
const FRAMEWORK_IGNORES: Record<string, string[]> = {
  ...
  // Patterns are matched with startsWith, so the fullstack entry can't reuse the
  // single-service lists above — every pattern needs its service prefix. Paths
  // mirror the services declared by the fullstack config template (frontend/,
  // api/) and cover both Next.js layouts (app/ and src/app/).
  "fullstack-nextjs-dotnet": [
    "frontend/.next/",
    "frontend/next.config",
    "frontend/src/app/layout.",
    "frontend/src/app/page.",
    "frontend/src/app/not-found.",
    "frontend/src/app/loading.",
    "frontend/src/app/error.",
    "frontend/src/app/global-error.",
