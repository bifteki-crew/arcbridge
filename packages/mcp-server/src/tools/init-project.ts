  ...
  server.tool(
    ...
    async (params) => {
      ...
      const summary = [
        ...
        "3. **Tailor building blocks** — edit `05-building-blocks.md`: delete blocks that don't apply, add blocks for your real modules, and declare `interfaces` between blocks (drift detection depends on this)",
        "4. **Tailor quality scenarios** — edit `10-quality-scenarios.yaml`: delete irrelevant scenarios, add ones that match your actual requirements",
        "5. **Tailor phase tasks** — Phase 0-1 tasks are ready to use. Phase 2+ tasks are examples only — **delete them** and create real tasks from the project's requirements using `arcbridge_create_task`. Add more phases with `arcbridge_create_phase` if needed.",
