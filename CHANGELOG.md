...
First release of ArchLens — an MCP server and CLI that gives AI coding agents architectural awareness via arc42 documentation.

### Packages

- `@archlens/core` — schemas, SQLite database, TypeScript indexer, drift detection, generators, testing runner, role/config loaders
- `@archlens/adapters` — Claude Code and GitHub Copilot config generators
- `@archlens/mcp-server` — MCP server with 26 tools for architecture, planning, code intelligence, and sync
- `archlens` — CLI with `init`, `sync`, `status`, `drift`, `update-task`, and `generate-configs` commands
...
- **Quality scenario verification** — link tests to scenarios, run them via `verify_scenarios` or `archlens sync`
