...
- **Fullstack building blocks hedged their paths, producing findings nobody could resolve.** Each block declared *both* layout variants (`frontend/app/` **and** `frontend/src/app/`), but only one can exist — and `missing_module` checks each `code_path` independently, correctly, since a block owning two real directories should report when one disappears. The unused variant therefore reported a permanent phantom warning in every fullstack project. Blocks now declare a single resolved path: detected from the service's own directory when it exists (the previous detector probed the repo root, so for a monorepo service it looked in the wrong place entirely), otherwise the framework's default. If the model and the code later disagree, that now surfaces as a matched `missing_module` + `undocumented_module` pair naming the real location — wrong and actionable, rather than hedged and permanent.
